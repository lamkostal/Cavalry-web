import { createFile } from 'https://unpkg.com/mp4box@2.3.0/dist/mp4box.all.js'

const DEBUG_VIDEO_TRACK_INFO = true
const DEBUG_DECODE_QUEUE = false
const DEBUG_DECODE_DELIVERY = false
const DEBUG_PREFETCH_STALL = false
const PREFETCH_STALL_MS = 500
const PREFETCH_RETRY_MS = 200
const SEEK_INTERRUPT_GAP = 15
 ...
 *    - Decoded frames are sent to Cavalry via instance.player.onPrefetchFrame()
 *    - Cavalry requests additional frames via instance.module.prefetchFrames()
 *    - This creates a pull-based streaming system
 *
 * == Callback Flow ==
 *
 * Frontend → Backend:
 *   instance.player.onPrefetchFrame(assetId, frameIndex, pixelDataPtr, width, height)
 ...
 *
 * Backend → Frontend:
 *   instance.module.prefetchFrames(assetId, startFrame, count)
 ...
 * @returns {HTMLCanvasElement|OffscreenCanvas|null} Canvas element
 */
function ensureCanvasForAsset(asset, width, height) {
	...
	const needsResize =
		!needsCanvas &&
		(asset.canvas.width !== width || asset.canvas.height !== height)
 ...
 *
 * @param {Object} instance - Cavalry player instance with `module` and `player` properties
 ...
 */
export async function loadWebCodecVideoAsset(instance, assetId, videoUrl) {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to load video. HTTP status: ${response.status} for video ${videoUrl}`,
			)
		}
	} catch (err) {
	...
	return new Promise((resolve, reject) => {
		...
		mp4boxFile.onReady = (info) => {
			...
			if (videoTrack.avg_frame_rate) {
				if (
					typeof videoTrack.avg_frame_rate === 'string' &&
					videoTrack.avg_frame_rate.includes('/')
				) {
			...
			const ptr = instance.module._malloc(frameBytes)
			instance.module.HEAPU8.fill(0, ptr, ptr + frameBytes)
			...
			if (DEBUG_VIDEO_TRACK_INFO) {
				...
				console.log(
					` - FPS (estimated from duration/samples): ${videoTrack.nb_samples}/(${videoTrack.duration}/${videoTrack.timescale}) = ${estimatedFPS}`,
				)
			 ...
			 * 3. Calls instance.player.onPrefetchFrame() to transfer the frame to Cavalry
			 *
			...
			const decoderOutput = async (frame) => {
				...
				if (timestampMap && typeof frame.timestamp === 'number') {
					...
					if (
						Array.isArray(entriesForTimestamp) &&
						entriesForTimestamp.length
					) {
						...
						if (DEBUG_DECODE_QUEUE && queueEntry) {
							console.log(
								`[VideoDecoder] asset=${assetId} matched queue entry by timestamp=${frame.timestamp}µs -> frame=${queueEntry.frameIndex}`,
							)
						}
					}
				}
				...
				if (
					!queueEntry &&
					Array.isArray(asset.decodeQueue) &&
					asset.decodeQueue.length
				) {
				...
				if (queueEntry && typeof queueEntry === 'object') {
					if (
						queueEntry.requestId != null &&
						queueEntry.requestId !== asset.requestId
					) {
						if (DEBUG_DECODE_DELIVERY) {
							console.log(
								`[VideoDecoder] asset=${assetId} dropping stale frame=${queueEntry.frameIndex} request=${queueEntry.requestId} active=${asset.requestId}`,
							)
						}
						frame.close()
						return
					...
					targetFrameIndex =
						typeof queueEntry.frameIndex === 'number'
					...
					if (queueEntry.deliver === false) {
						if (DEBUG_DECODE_DELIVERY) {
							console.log(
								`[VideoDecoder] asset=${assetId} skipping non-deliverable frame=${targetFrameIndex} timestamp=${frame.timestamp}`,
							)
						}
						frame.close()
						return
				...
				if (!queueEntry) {
					console.warn(
						`[VideoDecoder] asset=${assetId} decoded frame with no queue metadata timestamp=${frame.timestamp}`,
					)
				}

				// During initial prefill, stop processing once we have enough frames
				if (
					asset.isInitialPrefill &&
					asset.currentPrefillIndex >= asset.initialPrefillFrames
				) {
				...
				const defaultWidth =
					frame.displayWidth || frame.codedWidth || codedWidth
				const defaultHeight =
					frame.displayHeight || frame.codedHeight || codedHeight
				...
				if (asset.useStrideSize) {
					...
					try {
						...
						if (!asset.useStrideSize) {
							console.warn(
								'frame.allocationSize failed, switching to stride-based size.',
								allocationErr,
							)
				...
				if (!asset.ptr || asset.frameBytes < requiredBytes) {
					if (asset.ptr) {
						instance.module.HEAPU8.fill(
							0,
							asset.ptr,
							asset.ptr + asset.frameBytes,
						)
						instance.module._free(asset.ptr)
					}

					asset.ptr = instance.module._malloc(requiredBytes)
					asset.frameBytes = requiredBytes
				}

				const wasmBuffer = new Uint8Array(
					instance.module.HEAPU8.buffer,
					asset.ptr,
					requiredBytes,
				)
				...
				if (asset.copyMethod === 'canvas') {
					...
					try {
						const canvasPixels = copyFrameViaCanvas(
							frame,
							rect,
							frameWidth,
							frameHeight,
					...
					try {
						...
						if (!asset.copyMethod) {
							console.warn(
								'VideoFrame.copyTo failed, switching to canvas fallback.',
								err,
							)
				...
				if (copySucceeded) {
					...
					instance.player.onPrefetchFrame(
						assetId,
						targetFrameIndex,
						asset.ptr,
						frameWidth,
					...
					if (asset.lastPrefetch) {
						const { start, end } = asset.lastPrefetch
						if (targetFrameIndex >= start && targetFrameIndex <= end) {
							asset.lastPrefetch.delivered++
							if (asset.lastPrefetch.delivered >= asset.lastPrefetch.count) {
								if (asset.lastPrefetch.timer) {
									clearTimeout(asset.lastPrefetch.timer)
								}
								asset.lastPrefetch = null
							}
						}
					}
				} else {
					console.error(
						`[VideoDecoder] Failed to copy decoded frame asset=${assetId} frame=${targetFrameIndex}`,
					)
					if (asset.ptr) {
						instance.module.HEAPU8.fill(
							0,
			...
			instance.player.createWebAsset(assetId, false)
			instance.player.setVideoAssetInfo(
				assetId,
				totalFrames,
				fps,
				initialPrefillFrames,
			...
			mp4boxFile.onSamples = async (id, user, sampleList) => {
				...
				for (const sample of sampleList) {
					...
					const timestampMicros = convertTimestampToMicros(
				...
				if (
					!asset.prefillRequested &&
					hasFramesForRange(asset, 0, initialPrefillFrames)
				) {
 ...
 * When Cavalry needs frames, it calls instance.module.prefetchFrames() which triggers feedBatch().
 *
 ...
 *
 * @param {Object} instance - Cavalry player instance
 */
export function setupWebCodecHooks(instance) {
	instance.module.prefetchFrames = (assetId, start, count) => {
		const asset = window._videoAssets?.[assetId]
		if (asset) {
			asset.isInitialPrefill = false
			if (asset.lastPrefetch) {
				const seekGap = Math.abs(start - asset.lastPrefetch.start)
				const isSeek =
					start > asset.lastPrefetch.end + 1 ||
					start < asset.lastPrefetch.start - 1 ||
					seekGap > SEEK_INTERRUPT_GAP
				if (isSeek) {
					asset.requestId += 1
					asset.currentFetchRange = null
					asset.decodeQueue = []
					asset.decodeQueueByTimestamp = new Map()
					asset.pendingPrefetch = null
				}
			}
			asset.lastPrefetch = {
			...
			if (DEBUG_PREFETCH_STALL) {
				asset.lastPrefetch.timer = setTimeout(() => {
					if (!asset.lastPrefetch) return
					const elapsed = performance.now() - asset.lastPrefetch.requestedAt
					if (elapsed >= PREFETCH_STALL_MS && !asset.lastPrefetch.logged) {
						asset.lastPrefetch.logged = true
						console.warn(
							`[prefetchFrames] stalled asset=${assetId} request=${asset.lastPrefetch.start}-${asset.lastPrefetch.end} delivered=${asset.lastPrefetch.delivered}/${asset.lastPrefetch.count} elapsed=${Math.round(elapsed)}ms`,
						)
					}
				}, PREFETCH_STALL_MS + 50)
			}
		}
...
function convertTimestampToMicros(
	compositionTimestamp,
	timescale,
	decodeIndex,
	fps,
	...
	if (
		typeof compositionTimestamp === 'number' &&
		Number.isFinite(compositionTimestamp)
	) {
...
async function feedBatch(assetId, start, count) {
	...
	if (asset.currentFetchRange) {
		...
		if (!(requestEnd < fetchStart || start > fetchEnd)) {
			const lastPrefetch = asset.lastPrefetch
			if (lastPrefetch) {
				const elapsed = performance.now() - lastPrefetch.requestedAt
				if (elapsed > PREFETCH_RETRY_MS) {
					asset.currentFetchRange = null
					asset.decodeQueue = []
					asset.decodeQueueByTimestamp = new Map()
				} else {
					if (DEBUG_PREFETCH_STALL && !lastPrefetch.logged) {
						if (elapsed > PREFETCH_STALL_MS) {
							lastPrefetch.logged = true
							console.warn(
								`[feedBatch] stalled prefetch asset=${assetId} request=${lastPrefetch.start}-${lastPrefetch.end} active=${fetchStart}-${fetchEnd} elapsed=${Math.round(elapsed)}ms`,
							)
						}
					}
					return
	...
	if (
		!decoder ||
		!samples ||
		!Array.isArray(frameToDecode) ||
		frameToDecode.length === 0
	) {
	...
	if (!hasFramesForRange(asset, start, count)) {
		if (
			!asset.pendingPrefetch ||
			asset.pendingPrefetch.start !== start ||
			asset.pendingPrefetch.count !== count
		) {
	...
	try {
		...
		while (index < decodeEndExclusive) {
			...
			const frameIndexRaw = Array.isArray(decodeToFrame)
				? decodeToFrame[index]
				: sample.frameIndex
			const frameIndex =
				typeof frameIndexRaw === 'number' ? frameIndexRaw : null
			const timestampMicros =
				typeof sample.timestampMicros === 'number'
					? sample.timestampMicros
					: convertTimestampToMicros(
							sample.compositionTimestamp,
							timescale,
							index,
							asset.fps,
						)
			...
			if (DEBUG_DECODE_QUEUE) {
				const sampleNumber =
					typeof sample.number === 'number' ? sample.number : 'n/a'
				console.log(
					`[feedBatch] asset=${assetId} queued sample=${index} (number=${sampleNumber}) frame=${frameIndex ?? 'n/a'} deliver=${queueEntry.deliver} timestamp=${timestampMicros}`,
				)
			}
			...
			try {
				...
				console.error(
					`[VideoDecoder] decode() failed asset=${assetId} sample=${index} frame=${frameIndex} timestamp=${timestampMicros}`,
					err,
				)
			}
